// ============================================================================
//  src/main.js — ENTRY POINT for the VGN Generic Template shell
//  ----------------------------------------------------------------------------
...
import game from './examplegame.js';      // ← SWAP THIS for your game module
...
const CFG = {
  ...
  exampleOn:    cfg('exampleGameEnabled', true),
};
...
// The example module can be switched off from main.pjs — then the shell runs
// with a blank dark screen (no game hooked up yet).
const activeGame = CFG.exampleOn ? game : {
  init() {}, reset() {}, update() {},
...
const BOOT_LINES = [
  '> vgn-video-game-network  generic template v1.0.0',
  '> CRT DISPLAY ........... OK',
  '> INPUT DEVICE .......... OK',
...
$('howToNoteEl').textContent =
  "You're playing the template's EXAMPLE game (grab the coins). Build your own in " +
  'src/examplegame.js (or swap in a new module in src/main.js) — the shell keeps ' +
  'all of this: loop, pause, audio, HUD, high scores, CRT look.';
