...
// Combat theme — darker, unsettled (Em, F, Dm, E | ...)
const COMBAT_ROOTS = [41.2, 43.65, 36.71, 51.91, 41.2, 49, 36.71, 38.89];

function buildBass(roots) {
...
const BASS = buildBass(ROOTS);
const BASS_COMBAT = buildBass(COMBAT_ROOTS);
...
// Combative arpeggio for combat music
const ARP_COMBAT = [
  196, 233.08, 220, 261.63, 196, 233.08, 220, 311.13,
  196, 277.18, 220, 261.63, 164.81, 196, 220, 261.63,
];
...
const SFX = {
  ...
  hit: { type: 'square', vol: 0.09, notes: [
    { f: 180, d: 0.05, f2: 90 }, { f: 120, t: 0.05, d: 0.08, f2: 60 } ] },
  crit: { type: 'square', vol: 0.10, notes: [
    { f: 240, d: 0.06, f2: 120 }, { f: 300, t: 0.06, d: 0.1, f2: 150 } ] },
  hurt: { type: 'sawtooth', vol: 0.09, notes: [
    { f: 220, d: 0.1, f2: 110 }, { f: 150, t: 0.1, d: 0.12, f2: 80 } ] },
  spell: { type: 'sawtooth', vol: 0.08, notes: [
    { f: 500, d: 0.1, f2: 900 }, { f: 700, t: 0.09, d: 0.14, f2: 1200 } ] },
  heal: { type: 'sine', vol: 0.10, notes: [
    { f: 440, d: 0.14, f2: 660 }, { f: 660, t: 0.12, d: 0.18, f2: 880 } ] },
  chest: { type: 'square', vol: 0.08, notes: [
    { f: 660, d: 0.06 }, { f: 880, t: 0.07, d: 0.06 }, { f: 1100, t: 0.14, d: 0.12 } ] },
  stairs: { type: 'sawtooth', vol: 0.08, notes: [
    { f: 220, d: 0.12, f2: 330 }, { f: 330, t: 0.12, d: 0.12, f2: 440 },
    { f: 440, t: 0.24, d: 0.2, f2: 660 } ] },
  levelup: { type: 'square', vol: 0.09, notes: [
    { f: 523.25, d: 0.07 }, { f: 659.25, t: 0.08, d: 0.07 },
    { f: 783.99, t: 0.16, d: 0.07 }, { f: 1046.5, t: 0.24, d: 0.18 } ] },
  death: { type: 'sawtooth', vol: 0.10, notes: [
    { f: 300, d: 0.3, f2: 90 }, { f: 200, t: 0.32, d: 0.4, f2: 50 } ] },
  victory: { type: 'square', vol: 0.10, notes: [
    { f: 523.25, d: 0.1 }, { f: 783.99, t: 0.1, d: 0.1 },
    { f: 1046.5, t: 0.2, d: 0.1 }, { f: 783.99, t: 0.3, d: 0.1 },
    { f: 1318.5, t: 0.4, d: 0.35 } ] },
  bump: { type: 'square', vol: 0.05, notes: [ { f: 90, d: 0.05, f2: 60 } ] },
  rest: { type: 'sine', vol: 0.08, notes: [
    { f: 196, d: 0.2, f2: 165 }, { f: 165, t: 0.2, d: 0.3, f2: 130 } ] },
  flee: { type: 'square', vol: 0.07, notes: [
    { f: 660, d: 0.06, f2: 440 }, { f: 440, t: 0.06, d: 0.06, f2: 294 },
    { f: 294, t: 0.12, d: 0.08, f2: 196 } ] },
  encounter: { type: 'sawtooth', vol: 0.08, notes: [
    { f: 220, d: 0.07, f2: 440 }, { f: 440, t: 0.08, d: 0.07, f2: 660 } ] },
  search: { type: 'square', vol: 0.05, notes: [ { f: 330, d: 0.05 } ] },
};
...
export class AudioEngine {
  constructor() {
    ...
    this.muted = false;
    this.musicOn = false;
    this.theme = 'explore';          // 'explore' | 'combat' — swaps the music loop
  ...
  // Swap the looping music between the explore and combat themes. The RPG
  // calls this when a battle starts/ends to change the mood.
  setMusicTheme(theme) {
    if (theme === this.theme || !this.ctx) return;
    this.theme = theme;
    this._step = 0;
    this._nextStepTime = this.ctx.currentTime + 0.08;
  }
  ...
  _playStep(step, t) {
    ...
    const bass = combat ? BASS_COMBAT : BASS;
    const arp = combat ? ARP_COMBAT : ARP;
