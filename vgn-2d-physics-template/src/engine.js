...
export class GameEngine {
  constructor({ canvas, input, audio, hud, game, onTick, onStateChange }) {
    ...
    this.pointer = { x: LOGICAL_W / 2, y: LOGICAL_H / 2, down: false, active: false };
    this.gameCtx = {
      width: LOGICAL_W,
      height: LOGICAL_H,
      input,
      ...
      pointer: this.pointer,                   // mouse/touch, in logical px
    };

    this._size();
    window.addEventListener('resize', () => this._size());
    canvas.addEventListener('pointermove', (e) => this._setPointer(e));
    canvas.addEventListener('pointerdown', (e) => { this._setPointer(e); this.pointer.down = true; });
    window.addEventListener('pointerup', () => { this.pointer.down = false; });
    canvas.addEventListener('pointerleave', () => { this.pointer.active = false; });
  }

  // Track the pointer (mouse or touch) in LOGICAL_W × LOGICAL_H units. Games
  // read ctx.pointer.{x,y,down,active} — `active` stays true while the cursor
  // hovers the canvas, so paddle-control code can fall back to keyboard when
  // the player never touches the mouse.
  _setPointer(e) {
    const r = this.canvas.getBoundingClientRect();
    if (r.width === 0 || r.height === 0) return;
    this.pointer.x = ((e.clientX - r.left) / r.width) * LOGICAL_W;
    this.pointer.y = ((e.clientY - r.top) / r.height) * LOGICAL_H;
